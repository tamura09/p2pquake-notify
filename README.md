...
| `alert` | `/p2pquake-notify/discord/alert-webhook-url` | 緊急地震速報 (556) と津波予報 (552)。全国。加えて、速報を流した地震についてはその地震情報 (551) も。 |
...
### 速報を出した地震は、結果も同じチャンネルに出します

緊急地震速報 (556) は「これから揺れる」という予想でしかなく、実際にどこがどれだけ揺れたのかは後から出る地震情報 (551) にしか載りません。`alert` に速報だけを流していると「岩手県沿岸北部で震度5弱〜5強を予想」で話が終わり、本当に揺れたのかも分からないままになります。そこで **`alert` が緊急地震速報を流した地震については、code 551 を対象に含めていなくてもその地震情報を流します**（[eewQuakes](followup.go)、`route.QuakeAfterEEW`）。

突き合わせの鍵は**発生時刻**です。556 の `issue.eventId` は 551 に存在せず、両者を直接結ぶIDが無いためです。556 の `earthquake.originTime` は推定値、551 の `earthquake.time` は解析後の確定値なので、同じ地震でも数秒ずれます。一方で大きな地震の後は余震が数分おきに起きるため、窓を広げすぎると別の地震を同じものとして拾います。許容幅は2分 (`defaultEEWQuakeTolerance`)、地震情報を待つ期間は3時間 (`defaultEEWQuakeTTL`) にしています。

流れるのは 551 の全報なので、震度速報から各地の震度までは通常どおり1通にまとめられます（下記）。次の場合は流しません。

- 訓練報の緊急地震速報 (`test: true`)。`alert` にはそもそも流れないので、その地震情報も流れません。
- 取消報 (`cancelled: true`) だけが届いた地震。「揺れません」という報せなので、続きを待つ理由がありません。
- `originTime` が読めない速報。突き合わせようがないので、無関係な地震を流すより流さないほうを選びます。

覚えているのはメモリ上だけなので、**速報の後にタスクが再起動すると、その地震の地震情報は `alert` に出ません**。
